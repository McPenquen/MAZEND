...
void Level2Scene::Load() {
	...
	LevelScene::Load("res/maps/lvl2/botMap.txt", "res/maps/lvl2/midMap.txt", "res/maps/lvl2/topMap.txt");
	...
	movePlayerTo(Vector2f(_sectorBorders.left + tileBounds * 5.0f, _sectorBorders.top + tileBounds * 15.0f));
	// Set the active player
	setActivePlayer();

	// Collectable positions for this level
	MazePosition collectablePositions[COLLECTABLES_NUM] = {
		{Vector2f(_sectorBorders.right - tileBounds * 15.0f, _sectorBorders.bottom - tileBounds * 13.0f), Vector2i(1, 1), 3},
		{Vector2f(_sectorBorders.right - tileBounds * 13.0f, _sectorBorders.bottom - tileBounds * 13.0f), Vector2i(1, 3), 2},
		{Vector2f(_sectorBorders.left + tileBounds * 3.0f, _sectorBorders.bottom - tileBounds * 5.0f), Vector2i(2, 2), 1},
		{Vector2f(_sectorBorders.right - tileBounds * 7.0f, _sectorBorders.top + tileBounds * 13.0f), Vector2i(3, 2), 3},
		{Vector2f(_sectorBorders.left + tileBounds * 5.0f, _sectorBorders.bottom - tileBounds * 17.0f), Vector2i(2, 3), 3},
		{Vector2f(_sectorBorders.right - tileBounds * 3.0f, _sectorBorders.bottom - tileBounds * 9.0f), Vector2i(3, 3), 1},
		{Vector2f(_sectorBorders.right - tileBounds * 9.0f, _sectorBorders.bottom - tileBounds * 11.0f), Vector2i(2, 1), 2}
	};
	...
	enS->getShape().setFillColor(Color::Blue);
	enS->getShape().setOutlineColor(Color::White);
	enS->getShape().setOutlineThickness(2.f);
	enS->getShape().setOrigin(Vector2f(tileBounds / 2, tileBounds / 2));
	en->setPosition(Vector2f(Engine::GetWindowSize().x / 2 + 2.f, Engine::GetWindowSize().y / 2 + 0.0f));
